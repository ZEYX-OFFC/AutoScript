...
fun_menu() {
	...
	env_msg="━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>----------♻️ SATAN FUSION ♻️----------</b>\n"
	env_msg+="<b>  # ⚡️⚡️ Sithum Batrow 🇱🇰 ⚡️⚡      </b>\n"
	env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>/Telegram- @sibatrow /Whatsapp- +94710572224 </b>\n"
	env_msg+="━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
fun_ajuda() {
	...
	env_msg+="━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        env_msg+="[<b>01</b>] /menu = Displays menu\n"
        env_msg+="[<b>02</b>] /info = Displays information\n"
        env_msg+="[<b>03</b>] /ajuda = Option information\n\n"
        env_msg+="⚠️ <i>Available Options</i>\n\n"
        env_msg+="• <u>CREATE USER</u> = Create ssh user\n\n"
        env_msg+="• <u>CREATE TEST</u> = Create ssh test\n\n"
        env_msg+="• <u>TO REMOVE</u> = Remove ssh user\n\n"
        env_msg+="• <u>USER INFO</u> = User information\n\n"
        env_msg+="• <u>ONLINE USERS</u> = Displays online users\n\n"
        env_msg+="• <u>INFO VPS</u> = Server information\n\n"
        env_msg+="• <u>CHANGE PASSWORD</u> =Change ssh password\n\n"
        env_msg+="• <u>CHANGE LIMIT</u> = Change ssh limit\n\n"
        env_msg+="• <u>CHANGE DATE</u> = Change ssh data\n\n"
        env_msg+="• <u>EXPIRED</u> = Remove expired ssh\n\n"
        env_msg+="• <u>BACKUP</u> = Creates ssh backup and resellers\n\n"
        env_msg+="• <u>OTIMIZAR</u> = Clear the cache - ram\n\n"
        env_msg+="• <u>SPEEDTESTE</u> = connection test\n\n"
        env_msg+="• <u>FILES</u> = Hosts Files\n\n"
        env_msg+="• <u>RESELLERS</u> = Manage Resellers\n\n"
        env_msg+="• <u>AUTOBACKUP</u> = on / off automatic backup\n\n"
        env_msg+="• <u>REPORT</u> = Reseller Information\n\n"
        env_msg+="• <u>HELP</u> = Option information\n\n"
...
criar_user() {
    ...
    echo "$senha" >/etc/SatanFusion/senha/$usuario
...
fun_del_user() {
    ...
    [[ "${message_from_id[$id]}" = "$id_admin" ]] && {
        ...
        rm /etc/SatanFusion/senha/$usuario >/dev/null 2>&1
    } || {
    ...
    [[ -e /etc/SatanFusion/userteste/$usuario.sh ]] && rm /etc/SatanFusion/userteste/$usuario.sh
    ...
    if [[ "${callback_query_from_id[$id]}" == "$id_admin" ]]; then
        ...
        fun_infu() {
            ...
            for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
                info='===========================\n'
                [[ -e /etc/SatanFusion/senha/$user ]] && senha=$(cat /etc/SatanFusion/senha/$user) || senha='Null'
...
backup_users() {
    ...
    tar cvf /root/backup.vps /root/usuarios.db /etc/shadow /etc/passwd /etc/group /etc/gshadow /etc/bot /etc/SatanFusion/senha 1>/dev/null 2>/dev/null
...
sobremim() {
    local msg
    msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
    msg+="<b>🤖 BOT DARKSSH-MANAGER 🤖</b>\n"
    msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    msg+="<b>Desenvolvido por:</b> @crazy_vpn\n"
    msg+="<b>support:</b> @sibatrow\n\n"
    msg+="I was created with the purpose of providing information and tools for VPN management on 🐧 GNU / Linux servidores servers.\n\n"
    msg+="<b>Menu:</b> /menu\n"
    ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$(echo -e $msg)" \
        --parse_mode html
...
fun_teste() {
    ...
    cat <<-EOF >/etc/SatanFusion/userteste/$usuario.sh
	#!/bin/bash
	# USUARIO TESTE
	...
	rm /etc/SatanFusion/senha/$usuario > /dev/null 2>&1
	rm /etc/SatanFusion/userteste/$usuario.sh
	EOF
    chmod +x /etc/SatanFusion/userteste/$usuario.sh
    echo "/etc/SatanFusion/userteste/$usuario.sh" | at now + $t_time hour >/dev/null 2>&1
...
fun_exp_user() {
    if [[ "${callback_query_from_id[$id]}" == "$id_admin" ]]; then
        ...
        for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
            ...
            [[ -e /etc/SatanFusion/userteste/$user.sh ]] && rm /etc/SatanFusion/userteste/$user.sh
...
fun_backauto() {
    [[ "${callback_query_from_id[$id]}" = "$id_admin" ]] && {
        [[ ! -d /etc/SatanFusion/backups ]] && {
            mkdir /etc/SatanFusion/backups
            ...
            [[ -d /etc/SatanFusion/backups ]] && rm -rf /etc/SatanFusion/backups
            ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
...
backup_auto() {
    ShellBot.sendDocument --chat_id $id_admin \
        --document "@/etc/SatanFusion/backups/backup.vps" \
        --caption "$(echo -e "♻️ AUTOMATIC BACKUP ♻️")"
    rm /etc/SatanFusion/backups/backup.vps
    return 0
}
...
msg_bem_vindo() {
    local msg
    msg="✌️😃 Hi <b>${message_from_first_name[$id]}</b>\n\nWELCOME(a)\n\n"
    msg+="To access the menu\nclick or execute [ /menu ]\n\n"
    msg+="For information\nclick or execute [ /ajuda ]\n\n"
    ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$(echo -e $msg)" \
        --parse_mode html
    return 0
...
ShellBot.InlineKeyboardButton --button 'menu1' --line 6 --text 'USERS BACKUP' --callback_data '_backupusers'
ShellBot.InlineKeyboardButton --button 'menu1' --line 7 --text "HIGH BACKUP" --callback_data '_autobkp'
...
ShellBot.InlineKeyboardButton --button 'menu2' --line 2 --text 'TO REMOVE' --callback_data '_deluser2'
...
ShellBot.InlineKeyboardButton --button 'menu3' --line 2 --text 'TO REMOVE' --callback_data '_deluser3'
...
while :; do
    [[ -e "/etc/SatanFusion/backups/backup.vps" ]] && {
        backup_auto
    }
                ...
                '♾ CHANGE RESALE LIMIT ♾\n\nInform his user [Ex: @sibatrow]:')
                    ...
                    [[ $(grep -w ${message_from_username} $ativos | awk '{print $NF}') == 'revenda' ]] && {
                        [[ "$(grep -wc "$_Var1" /etc/bot/revenda/${message_from_username}/${message_from_username})" != '0' ]] && {
                            ...
                            ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                                --text "$(echo -e "❌ Sub-reseller does not exist")" \
                                --parse_mode html
                            break
                        }
                    }

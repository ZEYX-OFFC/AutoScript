...
fun_botOnOff () {
      [[ $(ps x | grep "bot_plus"|grep -v grep | wc -l) = '0' ]] && {
          clear
          echo -e "\E[44;1;37m             BOT SATAN FUSION INSTALLER                \E[0m\n"
          echo -ne "\033[1;32mINFORM YOUR TOKEN:\033[1;37m "; read tokenbot
  	    echo ""
  	    echo -ne "\033[1;32mINFORM YOUR ID:\033[1;37m "; read iduser
          clear
          echo -e "\033[1;32mSTARTING SATAN FUSION BOT \033[0m\n"
          fun_bot1 () {
  	        [[ ! -e "/etc/SatanFusion/ShellBot.sh" ]] && wget -qO- https://raw.githubusercontent.com/shellscriptx/shellbot/master/ShellBot.sh > /etc/SatanFusion/ShellBot.sh
  	        cd /etc/SatanFusion
  	        screen -dmS bot_plus ./bot $tokenbot $iduser > /dev/null 2>&1
              [[ $(grep -wc "bot_plus" /etc/autostart) = '0' ]] && {
		          echo -e "ps x | grep 'bot_plus' | grep -v 'grep' && echo 'ON' || cd /etc/SatanFusion && sudo screen -dmS bot_plus ./bot $tokenbot $iduser && cd $HOME" >> /etc/autostart
		      } || {
          ...
          fun_bar 'fun_bot2'
          echo -e "\n\033[1;32m \033[1;31mSATAN FUSION BOT STOPPED! \033[0m"
          sleep 3
          menu
      }
}
...
[[ -f "/etc/SatanFusion/ShellBot.sh" ]] && fun_botOnOff || fun_instbot
#fim